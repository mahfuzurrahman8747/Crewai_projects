All 72 unit tests pass and the Gradio app still validates cleanly. Here is the final test file:
...
class TestReporting(unittest.TestCase):
    ...
    def test_calculate_profit_or_loss_positive(self):
        # Start from an empty account, deposit exactly enough to buy, then
        # reprice upward -- this isolates the "pure price-appreciation"
        # contribution to P/L.
        a = Account(0)
...
**Summary of test results:** All **72 tests pass** (0 failures, 0 errors) when run with `python -m unittest test_account.py -v`. The suite covers:

- `prices.get_share_price` — known symbols, case-insensitivity, unknown/empty/non-string rejection (5 tests)
- `Account` construction — initial deposit, zero handling, default, negative rejection, opening-deposit log (5 tests)
- Deposits & withdrawals — cash movement, transaction logging, exact-balance, over-withdraw, non-positive rejection, exception hierarchy (12 tests)
- Buying shares — cash/holdings updates, logging, case-insensitivity, accumulation, insufficient funds, unknown symbol, bad symbol/quantity, non-int (including `bool`) rejection, state-unchanged-on-error (13 tests)
- Selling shares — cash/holdings updates, logging, case-insensitivity, full-sell removal, insufficient shares, unknown symbol, bad inputs, state-unchanged-on-error (12 tests)
- Reporting — empty/copy holdings, zero-quantity omission, portfolio value (cash-only / with-holdings / current-prices), P/L (break-even / positive / negative / withdrawal impact) (10 tests)
- Transaction history — copy, chronological ordering, completeness, correct `cash_after`, timestamp strings (5 tests)
- `Transaction.to_row` and `to_holdings_rows` / `to_transaction_rows` (9 tests)
- Full lifecycle integration + `AccountError` catchability (2 tests)

No changes to `prices.py`, `account.py`, or `app.py` were needed; the existing backend code already satisfies every contract the tests assert. The Gradio 6 app in `app.py` continues to import, construct, and exercise the backend cleanly (validated via `_validate.py`).